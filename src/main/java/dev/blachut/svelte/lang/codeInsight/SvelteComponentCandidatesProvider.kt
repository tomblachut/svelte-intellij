...
class SvelteComponentCandidatesProvider(placeInfo: JSImportPlaceInfo) : JSImportCandidatesBase(placeInfo) {
    override fun processCandidates(ref: String, processor: JSCandidatesProcessor, forCompletion: Boolean): Boolean {
        ...
        val virtualFile = FilenameIndex.getAllFilesByExt(project, "svelte").find { file ->
            if (file.name == "index.svelte") {
                file.parent.name == ref
            }
            else {
                file.nameWithoutExtension == ref
            }
        }
        ...
        processor.processCandidate(
            object : JSSimpleImportCandidate(ref, null, myPlaceInfo.place, JSImportCandidate.SourceType.ES6, true) {
                override fun createDescriptors(): List<JSImportDescriptor> {
                    val baseImportDescriptor = ES6CreateImportUtil.getImportDescriptor(name, null, virtualFile, myPlaceInfo.place, true)
    ...
    private fun hasNodeModulesInPath(element: PsiElement): Boolean {
        val file = PsiUtilCore.getVirtualFile(element)
        return file != null && NodeModuleUtil.hasNodeModulesDirInPath(file, null)
    }

    override fun getNames(keyFilter: Predicate<String>): MutableSet<String> {
        // todo investigate, processCandidates is called after this one
        ...
        val componentFiles =
            FilenameIndex.getAllFilesByExt(project, "svelte")
                ...
                }.filter(keyFilter).collect(Collectors.toSet())
