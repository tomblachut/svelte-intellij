...
/**
 * TypeScript-aware mutable variant for the bare `{let ...}` tag. Supports type annotations
 * (`{let foo: Type = expr}`). Reassignable, so [isConst] is `false`.
 */
class SvelteTSLetTagVariable(node: ASTNode) : TypeScriptVariableImpl(node) {
